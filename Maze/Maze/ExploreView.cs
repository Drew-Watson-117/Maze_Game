...
namespace Maze
{
    internal class ExploreView : GameView
    {
        Maze maze;
        Player player;
        ...
        public ExploreView(State currentState, List<Score> highScores, GraphicsDeviceManager graphics, ContentManager content) : base(graphics, content, currentState)
        {
            nextState = currentState;
            random = new Random();

            xDim = random.Next(100,250);
            yDim = random.Next(100,250);
        ...
        public override void Initialize()
        {
            ...
            maze.setStart(random.Next(0,xDim/2), random.Next(0,yDim));
            maze.setEnd(random.Next(xDim/2,xDim), random.Next(0,yDim));
            shortestPath = maze.shortestPath(maze.start, maze.end);
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            int offset = 50;
            int mazeStartX = _graphics.PreferredBackBufferWidth / 2 - offset * player.cell.x;
            int mazeStartY = _graphics.PreferredBackBufferHeight / 2 - offset * player.cell.y;
            ...
            for (int row = 0; row < xDim; row++)
            {
                for (int col = 0; col < yDim; col++)
                {
                    ...
                    if (!maze.neighborVisited(cell) && !cell.visitedByPlayer && cell != maze.end) // If neighboring tiles not explored, draw fog. Else, draw other stuff
                    {
